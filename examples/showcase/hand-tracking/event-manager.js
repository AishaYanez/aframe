...
AFRAME.registerComponent('event-manager', {
  ...
  init: function () {
    ...
    this.floatSphereGeometryEl = document.querySelector('#floatSphereGeometry');
    this.floatSmallSphereGeometryEl = document.querySelector('#floatSmallSphereGeometry');
    ...
    this.bombButtonEl = document.querySelector('#bombButton');
    this.floatButtonEl = document.querySelector('#floatButton');

    this.buttonToGeometry = {
      'boxButton': this.boxGeometryEl,
      'sphereButton': this.sphereGeometryEl,
      'torusButton': this.torusGeometryEl,
      'bombButton': this.bombGeometryEl,
      'floatButton': this.floatGeometryEl
    };
    ...
    this.bombButtonEl.addEventListener('click', this.onClick);
    this.floatButtonEl.addEventListener('click', this.onClick);
    this.floatSphereGeometryEl.addEventListener('click', this.onClick);
    this.floatSmallSphereGeometryEl.addEventListener('click', this.onClick);
  ...
  onClick: function (evt) {
    ...
    if (targetEl === this.boxButtonEl ||
        ...
        targetEl === this.bombButtonEl ||
        targetEl === this.floatButtonEl) {
      this.boxButtonEl.removeState('pressed');
      this.sphereButtonEl.removeState('pressed');
      this.torusButtonEl.removeState('pressed');
      this.bombButtonEl.removeState('pressed');
      this.floatButtonEl.removeState('pressed');
      ...
      this.bombGeometryEl.object3D.visible = false;
      this.floatGeometryEl.object3D.visible = false;
      this.buttonToGeometry[targetEl.id].object3D.visible = true;
    }

    if (targetEl === this.floatSphereGeometryEl ||
      targetEl === this.floatSmallSphereGeometryEl) {
      this.floatGeometryEl.object3D.visible = false;
    }
